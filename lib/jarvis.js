...
Jarvis.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        if (message.text.toLowerCase().indexOf("arquitectura") > -1 || message.text.toLowerCase().indexOf("software") > -1) {
            this.postMessageToChannel(channel.name, "https://i.imgsafe.org/51073548d3.png", {as_user: true});
        }
        if(message.text.toLowerCase().indexOf("no tengo ni idea") > -1){
            this.postMessageToChannel(channel.name, "http://i1.kym-cdn.com/photos/images/facebook/000/234/765/b7e.jpg", {as_user: true});
        }
...
Jarvis.prototype._welcomeMessage = function () {
    this.postMessageToUser("nhemesy","J.A.R.V.I.S. OPERATIVE"+this.user.name,{as_user: true});
};
